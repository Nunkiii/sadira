({
	...
	*/
	...
	if(ui_opts.show_cursor===true){
		...
		vec.cursor=vec.get('btns').add_child(cw, 'cursor');
	...
	var plot=function(data, xfunc, args){
		...
		if(p.le===undefined){
		    create_line_label(p, p.label, p.stroke).then(function(le){
			p.le=le;
		    });
		}else{
	...
	var points_plot=function(data, opts){
		...
		create_line_label(p,p.label, '#77f').then(function(ple){
	...
	var function_plot=function(func, opts){
	    ...
	    create_line_label(p,p.label, p.stroke).then(function(ple){
		p.le=ple;
	    });
	...
	vec.add_plot_points=function(data, opts){
		...
		points_plot(data, opts).then(function(p){
		    if(vec.value===undefined) vec.value=[];
		    vec.value.push(p);
		    //console.log("Added plot DL=" + p.data.length + " NP="+plots.length);
		    vec.config_range();
